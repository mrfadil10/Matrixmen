...
int	parse_colors(t_main *game, t_color *rgb, char **arr, bool *is_colored)
{
	...
	if ((rgb->r < 0 || rgb->r > 255) || (rgb->g < 0 || rgb->g > 255)
		|| (rgb->b < 0 || rgb->b > 255))
		return (set_err_msg(game, "\e[1;31mError: Invalid color range"));
	*is_colored = true;
	return (0);
}
...
int	parse_textures(t_main *game, char **dup, char **line, bool *is_tex)
{
	...
	while (line[++fd])
		;
	if (fd != 2)
		return (set_err_msg(game, "\e[1;31mError: Texture unknown error"));
...
int	identify_file_lines(t_main *game, char **arr)
{
	...
	return (set_err_msg(game, "\e[1;31mError: Invalid identifier"));
}
